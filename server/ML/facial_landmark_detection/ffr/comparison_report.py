...
def upload_to_s3(local_file, s3_file):
    """
    Upload a file to an S3 bucket using environment variables
    ...
    try:
        ...
        return True, s3_bucket, aws_region
    except FileNotFoundError:
        print(f"The file {local_file} was not found")
        return False, s3_bucket, aws_region
...
def generate_comparison_report(facial_metrics, username, gender):
    ...
    # Saving to JSON file
    comparison_report = df.to_dict(orient="records")
    with open("./reports/comparison_report.json", "w") as f:
        json.dump(comparison_report, f, indent=4)

    # Get environment variable for S3 prefix
    ffr_generated_prefix = os.environ.get('S3_FFR_PICTURES_GENERATED', 'ffr-pic-output/')
    s3_file_path = f"{ffr_generated_prefix}{username}_comparison_report.png"
    
    # Upload the graph to S3
    upload_success, s3_bucket, aws_region = upload_to_s3(viz_path, s3_file_path)

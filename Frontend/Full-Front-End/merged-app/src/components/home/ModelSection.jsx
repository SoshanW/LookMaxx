...
const ModelSection = () => {
  ...
  const animateUI = () => {
    ...
    if (femaleModelRef.current) {
      ...
      femaleTl.to(femaleModelRef.current, {
        scale: 1.05,
        duration: 0.3,
        ease: "power2.out",
      });

      femaleModelRef.current.addEventListener("mouseenter", () =>
        femaleTl.play()
      );
      femaleModelRef.current.addEventListener("mouseleave", () =>
        femaleTl.reverse()
      );
    }
    ...
    if (maleModelRef.current) {
      ...
      maleTl.to(maleModelRef.current, {
        scale: 1.05,
        duration: 0.3,
        ease: "power2.out",
      });
      ...
      maleModelRef.current.addEventListener("mouseleave", () =>
        maleTl.reverse()
      );
    }
  ...
  const setupModel = (
    ...
    loadingEl.innerHTML =
      '<div class="loading-spinner"></div><div>Loading Model...</div>';
        ...
        if (xhr.total > 0) {
          ...
          if (loadingEl && loadingEl.querySelector(".loading-spinner")) {
            loadingEl.querySelector(
              ".loading-spinner"
            ).style.background = `conic-gradient(#4f8fe6 ${percentComplete}%, #2a2a2a ${percentComplete}%)`;
          }
        }
      },
      // Error callback
  ...
  const startRendering = (
    ...
    const animate = (time) => {
      ...
      if (isVisible) {
        ...
        mouseState.currentX +=
          (mouseState.targetX - mouseState.currentX) * ease;
        mouseState.currentY +=
          (mouseState.targetY - mouseState.currentY) * ease;

        // Apply rotation based on mouse position if hovering
        if (mouseState.isHovering) {
          model.rotation.y =
            initialRotationValues[1] + mouseState.currentX * 0.4;
          model.rotation.x =
            initialRotationValues[0] + mouseState.currentY * 0.15;
        } else {
          // Gentle automatic rotation when not hovering
          // Use time for smooth rotation regardless of framerate
          model.rotation.y =
            initialRotationValues[1] + Math.sin(time * rotationSpeed) * 0.1;

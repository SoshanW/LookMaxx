...
def upload_file_to_s3(file, username):
    ...
    s3_prefix = os.environ.get('S3_FFR_PICTURES_UPLOAD', current_app.config.get('S3_FFR_PICTURES_UPLOAD'))
...
def upload_image_to_s3(img_array, filename, username, content_type='image/png'):
    ...
    ffr_generated_prefix = os.environ.get('S3_FFR_PICTURES_GENERATED', 
                                         current_app.config.get('S3_FFR_PICTURES_GENERATED'))
    s3_path = f"{ffr_generated_prefix}{username}/{filename}_ffr.png"
    s3_client = get_s3_client()
    s3_bucket = os.environ.get('S3_BUCKET', current_app.config.get('S3_BUCKET'))
